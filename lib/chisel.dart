...
import 'package:chisel/src/storage_io.dart' if (dart.library.html) 'package:chisel/src/storage_web.dart';
...
class Chisel {
  ...
  Future<void> generateModels({bool forceUpdate = false}) async {
    ...
    final metadata = await readMetadata(_metadataFilePath);
    ...
    await ensureDirectoryExists(fallbackDirectory);
    ...
    for (final table in schemaCache.values) {
      ...
      final filePath = getFilePath(_convertToUnderscoreCase(className), fallbackDirectory);
      await writeFile(filePath, classContent);
    }

    metadata['modelsGenerated'] = true;
    await writeMetadata(_metadataFilePath, metadata);
  }
