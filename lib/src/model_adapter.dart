  ...
  void _validateFields(dynamic fields) {
    ...
    for (var field in fieldNames) {
      if (!schema.hasColumn(field)) {
        throw ArgumentError('Field "$field" does not exist in table "$tableName".');
      }
    }
  }

Map<String, dynamic> _filterAutoGeneratedFields(Map<String, dynamic> parameters) {
  Logger.debug('Filtering auto-generated fields for table "$tableName"', context: getCallerContext());
  
  final schema = _chisel.getTableSchema(tableName);
  if (schema == null) {
    Logger.error('Table schema for "$tableName" not found in cache.', context: getCallerContext());
    throw Exception('Table schema for "$tableName" not found in cache.');
  }

  Logger.debug('Schema found for table "$tableName": ${schema.columns.map((col) => col.name).toList()}', context: getCallerContext());

  final filteredParameters = Map<String, dynamic>.from(parameters)
    ..removeWhere((key, _) {
      final column = schema.columns.firstWhere(
        (col) => col.name == key,
        orElse: () => throw ArgumentError('Field "$key" does not exist in table "$tableName".')
      );

      if (column.isAutoGenerated) {
        Logger.debug('Field "$key" is auto-generated and will be excluded.', context: getCallerContext());
        return true;
      }
      return false;
    });

  Logger.debug('Filtered parameters: $filteredParameters', context: getCallerContext());
  return filteredParameters;
}



  Future<T> create(Map<String, dynamic> parameters) async {
  try {
    Logger.info('Starting record creation in $tableName', context: getCallerContext());
    ...
    final result = await _chisel.instanceDBConnection.query(
      query, parameters: parameters,
      queryMode: QueryMode.extended,
      );

    if (result.isNotEmpty) {
      Logger.info('Record created successfully in $tableName', context: getCallerContext());
      return fromMap(result.first);
    } else {
      throw Exception('No rows returned after insertion.');
    }
  } catch (e, stackTrace) {
    Logger.error('Failed to create record in $tableName', context: getCallerContext());
  ...
  Future<List<T>> readAll({Map<String, dynamic>? filters}) async {
    ...
    final query = QueryBuilder.selectAll(tableName: tableName, filters: filters);
    final result = await _chisel.instanceDBConnection.query(query);
    return result.map((row) => fromMap(row)).toList();
  }

  Future<T> update(String fieldName, dynamic fieldValue, Map<String, dynamic> parameters) async {
    _validateFields(fieldName);
    ...
    throw Exception('Failed to update entry in $tableName.');
  }
  ...
  Future<void> delete(String fieldName, dynamic fieldValue) async {
    ...
    try {
      ...
      final dependencies = await _chisel.instanceDBConnection.query(dependenciesQuery);
